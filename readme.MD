...
> **Metodología:** _Spec-Driven Development_ (SDD) — **especificaciones → tests → implementación**.
...
- El devcontainer (`.devcontainer/devcontainer.json`) levanta automáticamente un servicio `postgres` (PostgreSQL 16) junto al contenedor de trabajo. Las credenciales por defecto son `agi_user` / `agi_pass` y la base `agi_force_mvp`; el script de inicialización vive en `docker/postgres/init/`.
- Dentro del Codespace tienes disponible `psql` y la variable `DATABASE_URL=postgres://agi_user:agi_pass@postgres:5432/agi_force_mvp`.
- El devcontainer instala la **Docker CLI** (`docker`, `docker compose`) para que puedas orquestar servicios adicionales sin salir del Codespace.
- Para reiniciar la base, ejecuta `docker compose -f .devcontainer/docker-compose.yml down -v` y luego `Dev Containers: Rebuild Container` (esto recrea los contenedores y volúmenes).
...
**Estilo:** _Arquitectura limpia / hexagonal_.
...
1. Crear app en <https://api.slack.com/apps> (**From scratch**, nombre: _PulseDesk_).
